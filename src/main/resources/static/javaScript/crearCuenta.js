...
function emailExists(email){
    let isValid = false;
    $.ajax({
        url:'http://158.101.3.197:8080/api/user/'+email,
        type: 'GET',
        dataType: 'json',
        async: false,
        ...
        error: function (){
            console.log("ERROR - No pudo verificar el e-mail")
        }
    });
    return isValid;
}
...
form.addEventListener('submit',function (event){
    ...
    if(!form.checkValidity() || !isValidName || !isValidEmail || !isValidPassword || !matchingPasswords || emailExists(email)){
        if(isValidName && emailExists(email)){
            document.getElementById('inputEmail').classList.add('is-invalid');
            document.getElementById('inputEmail').classList.remove('is-valid');
        ...
        $.ajax({
            url:'http://158.101.3.197:8080/api/user/new',
            type:'POST',
            dataType: 'json',
